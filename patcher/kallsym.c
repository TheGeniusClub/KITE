 ...
 * Reference from KernelPatch tools/kallsym.c
 */
...
int find_linux_banner(kallsym_t *info, char *img, int32_t imglen, void *opt)
{
    ...
    info->banner_num = 0;
    while ((banner = (char *)kite_memmem(banner + 1, imgend - banner - 1, linux_banner_prefix, prefix_len)) != NULL) {
...
static int find_token_table(kallsym_t *info, char *img, int32_t imglen, void *opt)
{
    ...
    for (; pos < imgend; pos = num_start + 1) {
        num_start = (char *)kite_memmem(pos, imgend - pos, nums_syms, sizeof(nums_syms));
        if (!num_start) {
        ...
        if (letter != (char *)kite_memmem(letter, sizeof(letters_syms), letters_syms, sizeof(letters_syms))) continue;
        break;
    }
...
static int find_token_index(kallsym_t *info, char *img, int32_t imglen, void *opt)
{
    ...
    char *lepos = (char *)kite_memmem(img, imglen, le_index, sizeof(le_index));
    char *bepos = (char *)kite_memmem(img, imglen, be_index, sizeof(be_index));
...
static int find_approx_addresses(kallsym_t *info, char *img, int32_t imglen)
{
    ...
    tools_logi("approximate kallsyms_addresses range: [0x%08x, 0x%08x) count: 0x%08x\n",
...
static int arm64_verify_pid_vnr(kallsym_t *info, char *img, int32_t offset)
{
    for (int i = 0; i < 6; i++) {
        ...
        uint32_t enc = (insn >> 25) & 0xF;
        if (enc == 0xD) {
            uint32_t sysreg = ((insn >> 5) & 0xFFFF) | ((insn >> 10) & 0x7C00);
            if (sysreg == 0xC208) {
